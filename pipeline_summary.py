...
fldr = sys.argv[1]
files = glob.glob1(fldr, '*_assocSNPs.csv') #assocSNPs is all extreme haplos
in_grp = sys.argv[2].replace('_', ' ').split(',')

hi, lo = pd.DataFrame(), pd.DataFrame()

for f in files:
    if '_lo_' in f: lo = pd.concat([lo, pd.read_csv(f"{fldr}/{f}", header=0)], axis=0)
    if '_hi_' in f: hi = pd.concat([hi, pd.read_csv(f"{fldr}/{f}", header=0)], axis=0)
...
try: hi_nfv[~hi_nfv.locus.isin(hi_alt)].drop_duplicates().to_csv(f'{fldr}/hiVal_uniqueResults.csv', index=False)
except: pass
try: lo_nfv[~lo_nfv.locus.isin(lo_alt)].drop_duplicates().to_csv(f'{fldr}/loVal_uniqueResults.csv', index=False)
except: pass

if not hi.empty:
    hi = hi.drop_duplicates()
    hi.to_csv(f"{fldr}/fullTable_highValGenos.csv", index=False)
...
if not lo.empty:
    ...
    lo.to_csv(f"{fldr}/fullTable_lowValGenos.csv", index=False)
...
df.to_csv(f"{fldr}/ALL_accession_cnts.csv", index=False)

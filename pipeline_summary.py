...
try: hi_nfv[~hi_nfv.locus.isin(hi_alt)].drop_duplicates().to_csv(f'{sys.argv[1]}/hiVal_uniqueResults.csv', index=False)
except: pass
try: lo_nfv[~lo_nfv.locus.isin(lo_alt)].drop_duplicates().to_csv(f'{sys.argv[1]}/loVal_uniqueResults.csv', index=False)
except: pass

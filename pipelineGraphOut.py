...
fig1 = sys.argv[5].replace('_', ' ').split(',') #NFV types 
fig2 = sys.argv[6].replace('_', ' ').split(',') #EC types 
...
colDik = {'white': (255, 255, 255), 'dkred': (0, 0, 150), 'blue': (200, 0, 0), 'red': (0, 0, 255), 'black': (1, 0, 0), 'green': (0, 200, 0), 'dkred': (0, 0, 150)}
try: prfx = f" - {sys.argv[11]}_"
except: prfx = ""
...
def main():
    ...
    chroms = [x.split('Os')[1].split('g')[0] for x in FILES]

    try: chroms = [sys.argv[12].zfill(2)] #single chrom for custom figure
    except: pass
    ...
    for chrom in sorted(set(chroms)):
        ...
        if hignz1.shape[0] < 68: im = rotateText(im, hiEnds1, pntr_len, pntr_wd, hignz1, 90, off, 2700, intersect)
        if lognz1.shape[0] < 68: im = rotateText(im, im.shape[1] - loEnds1, pntr_len, pntr_wd, lognz1, 270, off * 2, 1500, intersect)
        if hignz2.shape[0] < 68: im = rotateText(im, hiEnds2, pntr_len, pntr_wd, hignz2, 90, off, 1150, intersect)
        if lognz2.shape[0] < 68: im = rotateText(im, im.shape[1] - loEnds2, pntr_len, pntr_wd, lognz2, 270, off * 2, 3050, intersect)
...
def mkGene(im, start_end, h, h2, l, chrom):
    ...
    for gene in gene_list.index:
        try:
            if gene == int(sys.argv[6]) - 1:
                known = int(sys.argv[6])
                step, cnt = 1, 1
...
def rotateText(im, pointerLocs, pointerlen, pointerWid, genes, angle, off, d, intersect):
    ...
    for gene in genes:
        ...
        if gene in intersect: fontColor = colDik.get('red')
...
def unSquishTxt(im, pointerLocs):
    ...
    while swch != []:
        cntr += 1
        if cntr > 100: return np.array(pointerLocs)
        swch = []
